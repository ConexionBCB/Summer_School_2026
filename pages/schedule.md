---
layout: page
title: Programa
permalink: /schedule/
---

<p class="day-logo">
  <img src="{{ site.baseurl }}/assets/img/logos/logo_auxiliar.svg" alt="Conexión BCB" class="page-logo-small" />
</p>

Resumen de la semana. Cada día tiene su propia página con el horario detallado,
los temas y enlaces a los materiales teóricos y prácticos.

| Día       | Fecha      | Tema principal                                                                 | Enlace |
|-----------|------------|-------------------------------------------------------------------------------|--------|
| Lunes     | 20 julio   | Bienvenida, diseño de programas docentes y recursos para la docencia         | [Día 1 — Lunes]({{ site.baseurl }}/day1-monday/) |
| Martes    | 21 julio   | Ensamblado de genomas · AlphaFold y predicción de estructura de proteínas    | [Día 2 — Martes]({{ site.baseurl }}/day2-tuesday/) |
| Miércoles | 22 julio   | Transcriptómica de lecturas largas · Modelos para biología de sistemas       | [Día 3 — Miércoles]({{ site.baseurl }}/day3-wednesday/) |
| Jueves    | 23 julio   | Single‑cell · Transcriptómica espacial                                        | [Día 4 — Jueves]({{ site.baseurl }}/day4-thursday/) |
| Viernes   | 24 julio   | Integración multi‑ómica y cierre del curso                                   | [Día 5 — Viernes]({{ site.baseurl }}/day5-friday/) |
{: .schedule-table }

Para información sobre sede, requisitos previos y contacto, consulta la página de
[Logística]({{ site.baseurl }}/logistics/).
